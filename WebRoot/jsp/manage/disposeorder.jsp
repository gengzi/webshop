				...
				<div class="userCenterBox boxCenterList clearfix"
					...
					<center>
						<div id="fenye">
							...
							<!-- 首页 -->
							<a
								href='<c:url value='/OrderServlet?method=4&pagecode=1&flag=10'></c:url>'>首页</a>
							<!-- 上一页 -->
							<c:if test="${orderPageBean.pageCode>1 }">
								<!-- 当页码大于第一页 -->
								<a
									href='<c:url value='/OrderServlet?method=4&pagecode=${orderPageBean.pageCode-1}&flag=10'></c:url>'>上一页</a>
							</c:if>
							...
							<c:forEach var="i" begin="${begin}" end="${end}">
								...
								<c:choose>
									...
									<c:otherwise>
										<a
											href='<c:url value='/OrderServlet?method=4&pagecode=${i}&flag=10'></c:url>'>${i}</a>
									</c:otherwise>
								</c:choose>
							</c:forEach>
							...
							<c:if test="${orderPageBean.pageCode< orderPageBean.totalPage }">
								<!-- 当前页码不大于最大的页码 -->
								<a
									href='<c:url value='/OrderServlet?method=4&pagecode=${orderPageBean.pageCode+1 }&flag=10'></c:url>'>下一页</a>
							</c:if>
							<a
								href='<c:url value='/OrderServlet?method=4&pagecode=${orderPageBean.totalPage}&flag=10'></c:url>'>尾页</a>
